...
def my_all?(argument = nil)
   if block_given?
    self.my_each {|w| return false if yield(w) == false}
    elsif  [Integer, Float, String].include? (argument.class) 
    self.my_each {|w| return false unless w == argument}
   elsif argument.class == Regexp
    self.my_each {|w| return false unless w =~ argument} #the =~ operator matches the regular expression against a string
   elsif argument.class == Class
    self.my_each {|w| return false unless w.class == argument}
    else !argument && !block_given?
    self.my_each {|w| return false unless w}
   end
   true   
    
end
#  puts (['j', 'h'].my_all?("h"))   
#  puts ([0.2, 2].my_all?(Float))
...
def my_any? (argument = nil)
    if block_given?
        self.my_each {|w| return true if yield(w) == true}
    elsif [Integer, Float, String].include? (argument.class)
        self.my_each {|w| return true if argument == w}
    elsif argument.class == Regexp
        self.my_each {|w| return true if w =~ argument}
    elsif argument.class == Class
        self.my_each {|w| return true if w.class == argument}
    else !block_given? && !argument
        self.my_each {|w| return true if w}
    end
    false
end
...
def my_none? (argument = nil)
  if block_given?
    self.my_each {|w| return false if yield(w) == true}
  elsif [String, Float, Integer].include? (argument.class)
    self.my_each {|w| return false if w == argument}
  elsif argument.class == Regexp
    self.my_each {|w| return false if w =~ argument}
  elsif argument.class = Class
    self.my_each {|w| return false if w.class == argument}
  else !argument && !block_given?
    self.my_each {|w| return false if w}
  end
  true
end
...
def my_count(argument = nil)
  ...
  if block_given?
    ...
    self.my_each {|w|  x +=1 if argument == w}
    return x
  else
    self.my_each { x += 1}
    return x 
